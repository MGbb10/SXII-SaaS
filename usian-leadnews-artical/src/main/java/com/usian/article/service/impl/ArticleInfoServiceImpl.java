...
public class ArticleInfoServiceImpl implements ArticleInfoService {
    ...
    public ResponseResult loadArticleInfo(ArticleInfoDto dto) {
        ...
        //2.查询文章的配置
        ApArticleConfig apArticleConfig = apArticleConfigMapper.selectOne(Wrappers.<ApArticleConfig>lambdaQuery().eq(ApArticleConfig::getId, dto.getArticleId()));
        ...
        if (!apArticleConfig.getIsDelete()&&!apArticleConfig.getIsDown()){
            ApArticleContent apArticleContent = apArticleContentMapper.selectOne(Wrappers.<ApArticleContent>lambdaQuery().eq(ApArticleContent::getId, dto.getArticleId()));
